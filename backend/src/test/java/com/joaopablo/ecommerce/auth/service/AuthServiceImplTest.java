...
class AuthServiceImplTest {
    ...
    void issueTokensShouldReturnAccessAndRefreshTokens() {
        ...
        when(jwtService.generateToken("user@email.com")).thenReturn("access-123");
        when(jwtService.getExpirationMs()).thenReturn(86400000L);
        when(refreshTokenService.create(user)).thenReturn(refreshToken);

        LoginResponseDTO response = authService.issueTokens(user);

        assertEquals("access-123", response.getToken());
        ...
        assertEquals(Set.of("CUSTOMER"), Set.copyOf(response.getUser().getRoles()));
    }
    ...
    void refreshShouldRotateRefreshTokenAndIssueNewAccessToken() {
        ...
        when(jwtService.generateToken("user@email.com")).thenReturn("new-access");
        when(jwtService.getExpirationMs()).thenReturn(86400000L);

        TokenRefreshResponseDTO response = authService.refresh(
                RefreshTokenRequestDTO.builder().refreshToken("old-refresh").build()
        );
